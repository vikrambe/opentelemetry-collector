  ...
  node(label) {
    ...
    stage('Docker Build') {
       ...
       stage('CPD Certification') {
        withCredentials([usernamePassword(credentialsId: "twistlock-cpd-scan", passwordVariable: 'SCAN_PASSWORD', usernameVariable: 'SCAN_USER'), usernamePassword(credentialsId: "cloud-logging", passwordVariable: 'DOCKER_PASSWORD', usernameVariable: 'DOCKER_USERNAME')]) {                  
